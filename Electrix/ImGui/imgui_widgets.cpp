...
{
    bool need_backup = (GImGui->CurrentWindow->DC.TextWrapPos < 0.0f);           
    if (need_backup)
...
static ImGuiTabItem * ImGui::TabBarScrollingButtons(ImGuiTabBar * tab_bar)
{
    ...
    if (select_dir != 0)
        if (ImGuiTabItem * tab_item = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId))
        {
            ...
            tab_to_select = &tab_bar->Tabs[(target_order >= 0 && target_order < tab_bar->Tabs.Size) ? target_order : selected_order];                 
        }
...
bool    ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (ret && !(flags & ImGuiTabItemFlags_NoPushId))
    {
        ...
        g.CurrentWindow->IDStack.push_back(tab->ID);                      
    }
    return ret;
}
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList * draw_list, const ImRect & bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id)
{
    ...
    if (label_size.x > text_ellipsis_clip_bb.GetWidth())
    {
        ...
        if (label_end == label && label_end < label_display_end)                  
        {
        ...
        while (label_end > label && ImCharIsBlankA(label_end[-1]))    
        {
            label_end--;
            label_size_clipped_x -= g.Font->CalcTextSizeA(g.FontSize, FLT_MAX, 0.0f, label_end, label_end + 1).x;       
        }
